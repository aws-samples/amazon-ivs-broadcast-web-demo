...
export default function Broadcast() {
  ...
  const [cameraKitSession, setCameraKitSession] = useState(null);
  const [lenses, setLenses] = useState([]);
  const [selectedFilter, setSelectedFilter] = useState({ id: 0, name: "Disabled" });
  ...
  const previousFilterEnabled = useRef(false);
  ...
  const handleStream = async () => {
    if (ingestServer && streamKey) {
      toggleStream(ingestServer, streamKey, channelType.current, client.current, handleError, cameraKitSession);
    } else {
      handleSettings();
    }
  };
  ...
  const handleFilters = async () => {
    ...
    try {
      ...
      handleError(`Filter error: ${err.message}`);
    }
  };
  ...
  const handleScreenShare = async () => {
    ...
    try {
      if (captureStream?.active) {
        await stopScreenShare(activeVideoDevice.current, camMuted, removeLayer, removeMixerDevice, updateLayer, client.current);
      } else {
        await startScreenShare(activeVideoDevice.current, camMuted, updateLayer, addLayer, removeLayer, removeMixerDevice, addAudioTrack, canvas, client.current);
      }
    } catch (err) {
    ...
    if (!devicePermissions.video) {
      try {
        ...
        if (!devicePermissions.video && !devicePermissions.audio) {
          handleError("Failed to access the camera and microphone. To start streaming, you must allow access to both your camera and microphone");
        }
      }
    }
  ...
  useEffect(async () => {
    ...
    const { bootstrapCameraKit, createUserMediaSource, Transform2D } = require("@snap/camera-kit");

    // Handle Localstorage
    if (localStorage) {
      console.log("NEXT_PUBLIC_SNAP_CAMERA_KIT_API_TOKEN", process.env.NEXT_PUBLIC_SNAP_CAMERA_KIT_API_TOKEN);
      ...
      const { lenses } = await cameraKit.lensRepository.loadLensGroups([process.env.NEXT_PUBLIC_LENS_GROUP_ID]);
      setLenses(lenses);
      session.events.addEventListener("error", (event) => console.error("CameraKit error: ", event.detail));
  ...
  useEffect(() => {
    ...
    if (channelType) {
      ...
      switch (formatted) {
        ...
        default:
          console.error(`Channel type must be STANDARD, BASIC, or BASIC_FHD. The channel type you provided is ${channelType}. The default value of STANDARD has been set`);
          setChannelType("STANDARD");
          break;
      }
    }
  ...
  return (
    <>
      <div className={styles.broadcastWrapper}>
        <Head>
          <title>Amazon IVS – Web Broadcast Tool - {`${isLive ? "LIVE" : "Offline"}`}</title>
          <meta name="description" content="This tool can be used to stream your webcam and share your screen to an Amazon IVS Channel." />
          <link rel="icon" href="/favicon.ico" />
        </Head>
        <Script
          src="https://web-broadcast.live-video.net/1.3.2/amazon-ivs-web-broadcast.js"
        ...
        <AlertBar
          ...
          show={showAlert}
          message={"This browser is not fully supported by this tool. For best results, open this page on a PC or Mac in Google Chrome or Firefox."}
        />
      ...
      <Modal type={"full"} show={settingsModalActive} onClose={toggleSettingsModal}>
        <Settings
          ...
          activeVideoDeviceId={activeVideoDevice.current ? activeVideoDevice.current.deviceId : ""}
          activeAudioDeviceId={activeAudioDevice.current ? activeAudioDevice.current.deviceId : ""}
