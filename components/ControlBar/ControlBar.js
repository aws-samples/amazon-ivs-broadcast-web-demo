...
function SettingsButton({ handleSettings, ...additionalProps }) {
  return (
    <Button style="round" onClick={handleSettings} {...additionalProps}>
      <Icon>
        <svg width="48" height="48" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" id="cog">
...
function FiltersMenu({ handleFilters, setFilterEnabled, filterEnabled, setSelectedFilter, selectedFilter, lenses, cameraKitSession }) {
  ...
  return (
    <select value={selectedFilter.id} onChange={handleFilterChange}>
...
function StreamButton({ isLive, handleStream, loading, disabled, ...additionalProps }) {
  ...
  return (
    <Button type={`${buttonStyle}`} style="roundedText" onClick={handleStream} fullWidth={"responsive"} disabled={disabled} loading={loading} {...additionalProps}>
      {buttonContent}
    </Button>
  );
}
    ...
    <div className={styles.controlBar}>
      ...
      {isDesktop && (
        <Tooltip content={`${screenShareActive ? "Stop sharing" : "Share your screen"}`}>
          <ScreenShareButton active={screenShareActive} handleScreenShare={handleScreenShare} disabled={!videoPermissions} />
        </Tooltip>
      )}
      <Tooltip content="Open settings">
        <SettingsButton handleSettings={handleSettings} disabled={!videoPermissions} />
      </Tooltip>
      ...
      />
      <StreamButton isLive={isLive} handleStream={handleStream} loading={streamLoading} disabled={!videoPermissions || streamLoading} />
    </div>
  );
}
